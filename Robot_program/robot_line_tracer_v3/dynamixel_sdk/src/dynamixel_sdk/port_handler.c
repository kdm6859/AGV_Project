...
#include "port_handler.h"
#include "port_handler_cortex_m3.h"

int     portHandler         (const char *port_name) { return portHandlerCortexm3(port_name); }

uint8_t openPort            (int port_num) { return openPortCortexm3(port_num); }
void    closePort           (int port_num) { closePortCortexm3(port_num); }
void    clearPort           (int port_num) { clearPortCortexm3(port_num); }

void    setPortName         (int port_num, const char *port_name) { setPortNameCortexm3(port_num, port_name); }
char   *getPortName         (int port_num) { return getPortNameCortexm3(port_num); }

uint8_t setBaudRate         (int port_num, const int baudrate) { return setBaudRateCortexm3(port_num, baudrate); }
int     getBaudRate         (int port_num) { return getBaudRateCortexm3(port_num); }

int     readPort            (int port_num, uint8_t *packet, int length) { return readPortCortexm3(port_num, packet, length); }
int     writePort           (int port_num, uint8_t *packet, int length) { return writePortCortexm3(port_num, packet, length); }

void    setPacketTimeout    (int port_num, uint16_t packet_length) { setPacketTimeoutCortexm3(port_num, packet_length); }
void    setPacketTimeoutMSec(int port_num, double msec) { setPacketTimeoutMSecCortexm3(port_num, msec); }
uint8_t isPacketTimeout     (int port_num) { return isPacketTimeoutCortexm3(port_num); }
