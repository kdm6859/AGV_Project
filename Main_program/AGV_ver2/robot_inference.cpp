...
cv::Mat get_detected_img(Net network, cv::Mat input_image, float score_threshold, cv::Point* robot_point, cv::Rect* robot_rect, int robot_num) {
    ...
    for (int i = 0; i < detection_result.rows; i++) {
        ...
        if (score > score_threshold) {
            ...
            cv::Rect temp_rect = cv::Rect(cv::Point(left - 10, top - 10), cv::Point(right + 10, bottom + 10));
...
int robot_find_ID(cv::Mat frame, cv::Rect robot_rect) {
    ...
    //파랑 스티커
    cv::Scalar lower_blue = cv::Scalar(90, 100, 150);
    cv::Scalar upper_blue = cv::Scalar(120, 130, 200);
    //핑크 스티커
    cv::Scalar lower_pink = cv::Scalar(170, 60, 140);
    cv::Scalar upper_pink = cv::Scalar(179, 110, 180);

    cv::Mat blue_mat, pink_mat;

    inRange(robot_roi, lower_blue, upper_blue, blue_mat);
    inRange(robot_roi, lower_pink, upper_pink, pink_mat);

    int blue_num = 0, pink_num = 0;

    for (int r = 0; r < robot_roi.rows; r++) {
        for (int c = 0; c < robot_roi.cols; c++) {
            if (blue_mat.at<uchar>(r, c) > 0)
                blue_num++;
            if (pink_mat.at<uchar>(r, c) > 0)
                pink_num++;
        }
    }

    if (blue_num > pink_num)
        return 0;
    else
        return 1;
}
