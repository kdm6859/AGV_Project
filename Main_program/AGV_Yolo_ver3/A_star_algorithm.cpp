...
//Navigate_ver4
void A_star_algorithm::Navigate() {
	//std::cout << "Navigate() 시작" << std::endl;
	cv::Mat robot_map = map.clone();
	//std::cout << "Navigate() 1" << std::endl;
	...
	//std::cout << "Navigate() 2" << std::endl;
	//시작 노드 닫힌 노드에 삽입
	close_list.insert(close_list.size(), new Node(current_point));
	//std::cout << "Navigate() 3" << std::endl;
	float distance;
	while (1) {
		//현재 노드
		...
		Node* temp_Node = open_list.remove();
		//std::cout << temp_Node->getPoint() << std::endl;
		close_list.insert(close_list.size(), temp_Node);
	...
	while (path_node != NULL) {
		...
		cv::line(pure_path, path_point, n_p, cv::Scalar(100), 5, cv::LINE_4);
	}
