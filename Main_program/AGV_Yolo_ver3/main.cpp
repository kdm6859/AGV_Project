...
int main() {
	...
	while (1) {
		...
		//로봇 인식
		view = frame.clone();
		cv::Mat my_view = frame.clone();
		get_detected_robot(robot_net, num_net, direction_net, frame, my_view, 0.4,
			...
			robot_point, robot_rect, robot_head, ROBOT_NUM);

		for (int i = 0; i < ROBOT_NUM; i++) {
			cv::rectangle(view, cv::Point(robot_rect[i].x, robot_rect[i].y), cv::Point(robot_rect[i].x + robot_rect[i].width, robot_rect[i].y + robot_rect[i].height), cv::Scalar(0, 255, 255), 2);
			cv::circle(view, robot_point[i], 3, cv::Scalar(0, 0, 255), -1);
			...
			cv::circle(view, robot_head[i], 4, cv::Scalar(0, 255, 0), -1);
			...
			cv::putText(view, std::to_string(i), robot_point[i], cv::FONT_HERSHEY_COMPLEX_SMALL, 2, cv::Scalar(0, 255, 0));
		}

		//view = get_detected_img(network, frame, 0.8, robot_point, robot_rect, ROBOT_NUM,
		//	num_net, direction_net, num_output_names, direction_output_names,
		//	NUM_CLASSES_NUM, NUM_CLASSES_DIRECTION,
		//	CONFIDENCE_THRESHOLD_NUM, CONFIDENCE_THRESHOLD_DIRECTION,
		//	NMS_THRESHOLD_NUM, NUM_CLASSES_DIRECTION, colors, NUM_COLORS,
		//	num_class_names, direction_class_names, robot_head);
		...
		map = cv::Mat(360, 640, CV_8U, cv::Scalar(0));
