...
DWORD WINAPI RobotClient(LPVOID arg) {
	...
	path[Robot_ID] = cv::Mat(360, 740, CV_8S, cv::Scalar(-128));
	robot_path[Robot_ID] = cv::Mat(360, 740, CV_8S, cv::Scalar(-128));
	pure_path[Robot_ID] = cv::Mat(360, 740, CV_8U, cv::Scalar(0));
	...
	robot_map[Robot_ID] = cv::Mat(360, 740, CV_8U, cv::Scalar(0));
	while (1) {
...
int main() {
	...
	cv::Mat hsv_img;
	cv::Rect my_roi(0, 0, 740, 360);
	cv::Mat map_roi;

	cv::Mat robot_map(360, 740, CV_8U, cv::Scalar(0)); //map
	cv::Mat path(360, 740, CV_8S, cv::Scalar(-128));
	...
	while (1) {
		...
		//색 보정
		cv::Scalar frame0_mean = cv::mean(frame0);
		cv::Scalar frame1_mean = cv::mean(frame1);
		cv::Scalar average(((int)frame0_mean[0] + (int)frame1_mean[0]) / 2,
			((int)frame0_mean[1] + (int)frame1_mean[1]) / 2,
			((int)frame0_mean[2] + (int)frame1_mean[2]) / 2);

		cv::Mat frame0_rgb[3];
		cv::split(frame0, frame0_rgb);
		cv::Mat frame1_rgb[3];
		cv::split(frame1, frame1_rgb);

		for (int i = 0; i < 3; i++) {
			if (frame0_mean[i] > average[i]) {
				frame0_rgb[i] -= ((int)frame0_mean[i] - (int)average[i]);

			}
			else {
				frame0_rgb[i] += ((int)average[i] - (int)frame0_mean[i]);
			}
		}

		for (int i = 0; i < 3; i++) {
			if (frame1_mean[i] > average[i]) {
				frame1_rgb[i] -= ((int)frame1_mean[i] - (int)average[i]);

			}
			else {
				frame1_rgb[i] += ((int)average[i] - (int)frame1_mean[i]);
			}
		}

		cv::merge(frame0_rgb, 3, frame0);
		cv::merge(frame1_rgb, 3, frame1);

		/*std::cout << average << std::endl;
		std::cout << frame0_mean << std::endl;
		std::cout << frame1_mean << std::endl << std::endl;*/

		//스티칭(영상 합성)
		stitch_up = stitcher0.stitch(frame1, frame0, "width");

		stitch_up = stitch_up(my_roi).clone();

		//로봇 인식
		stitch_up = get_detected_img(network, stitch_up, 0.8, robot_point, robot_rect, ROBOT_NUM);
		...
		cv::Mat status_frame(360, 740, CV_8U, cv::Scalar(0));
		status_frame += map2;
		for (int i = 0; i < ROBOT_NUM; i++) {
			if (move_status[i])
