...
int main() {
	...
	cv::resize(my_map, my_map, cv::Size(), 15, 15, 0);
	...
	std::cout << "1" << std::endl;
	A_star_algorithm A_star(my_map, cv::Point(3 * 15, 3 * 15), cv::Point(7 * 15, 8 * 15));
	//A_star_algorithm A_star(my_map, cv::Point(3, 3), cv::Point(7, 8));
	//A_star_algorithm A_star(my_map, cv::Point(0, 0), cv::Point(99, 99));
	std::cout << "2" << std::endl;
	clock_t start, end;
	double time_result;
	...
	std::cout << "4" << std::endl;
	//cv::resize(n_map, n_map, cv::Size(),10, 10, 0);
	while (1) {
