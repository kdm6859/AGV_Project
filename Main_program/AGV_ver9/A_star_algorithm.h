...
class A_star_algorithm
{
	...
	cv::Mat map, res_map, robot_res_map, pure_path; //redmap, res_map2;
...
public:
	A_star_algorithm() {
		...
		pure_path = cv::Mat(map.rows, map.cols, CV_8U, cv::Scalar(0));
	}
	A_star_algorithm(cv::Mat map, cv::Point start_point, cv::Point goal_point) :
		size(0), result_distance(0), start_point(start_point), goal_point(goal_point), current_point(start_point) {
		this->map = map.clone();
		res_map = cv::Mat(map.rows, map.cols, CV_8U, cv::Scalar(0));
		//res_map2 = cv::Mat(map.rows, map.cols, CV_8S, cv::Scalar(0));
		robot_res_map = cv::Mat(map.rows, map.cols, CV_8S, cv::Scalar(0));
	...
	cv::Mat get_pure_path() { return pure_path; }
};
