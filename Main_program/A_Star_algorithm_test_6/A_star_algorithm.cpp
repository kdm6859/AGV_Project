...
void A_star_algorithm::Navigate() {
	...
	for (int i = 0; i < contours.size(); i++) {
		drawContours(robot_map, contours, i, robot_map_c, 20);
		drawContours(map, contours, i, robot_map_c, 20);
	}
	...
	std::cout << "@@" << std::endl;
	for (int i = 0; i < contours.size(); i++) {
		drawContours(res_map, contours, i, c, 10);
	}
	...
	while (1) {
		...
		if (path_node == NULL)
			break;
		cv::Point n_p = path_node->getPoint();
		cv::line(robot_res_map, path_point, n_p, cv::Scalar(0), 5, cv::LINE_4);
	}
