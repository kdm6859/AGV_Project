...
int main() {
	...
	char robot_command[10] = { 0 };
	char before_robot_command[10] = { 0 };
	double direction_angle;
	double robot_head_angle;

	//로봇 제어 관련
	int speed_value = 60, r_speed_value = 0, l_speed_value = 0;
	int gain = 30;
	...
	while (cap.isOpened()) {
		...
		if (robot_move) {
			...
			//else if (move_start) {
			//	//strcpy(robot_command, "?start!");
			//	strcpy_s(robot_command, "?s!");
			//	move_start = false;
			//}
			else {
				/*std::string robot_command_str = "";
				robot_command_str += "?";
				robot_command_str += std::to_string((int)robot_path.at<char>(robot_point.y, robot_point.x));
				robot_command_str += "!";
				strcpy_s(robot_command, robot_command_str.c_str());*/
				std::string robot_command_str = "";

				//로봇 중심위치로 판단
				/*if ((int)robot_path.at<char>(robot_point.y, robot_point.x) < 0)
					robot_command_str += std::to_string(-(int)robot_path.at<char>(robot_point.y, robot_point.x) + 4);
				else
					robot_command_str += std::to_string((int)robot_path.at<char>(robot_point.y, robot_point.x));*/
				//로봇 머리위치로 판단
				/*if ((int)robot_path.at<char>(robot_head.y, robot_head.x) < 0)
					robot_command_str += std::to_string(-(int)robot_path.at<char>(robot_head.y, robot_head.x) + 4);
				else
					robot_command_str += std::to_string((int)robot_path.at<char>(robot_head.y, robot_head.x));
				strcpy_s(robot_command, robot_command_str.c_str());*/
