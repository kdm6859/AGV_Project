...
CRITICAL_SECTION cs_input_command, cs_command_status, cs_robot_point[ROBOT_NUM]
	...
	, cs_goal_point, cs_map, cs_result_distance[ROBOT_NUM], cs_find_end
	, cs_thread_wake;
...
DWORD WINAPI RobotClient(LPVOID arg) {
	...
	while (1) {
		...
		//
		//	EnterCriticalSection(&cs_find_end);
		//	find_end++;
		//	LeaveCriticalSection(&cs_find_end);
		//	printf("ID : %d\n", Robot_ID);
		//
		//	//다른 쓰레드 탐색 종료 대기
...
int main() {
	...
	InitializeCriticalSection(&cs_find_end);
	...
	DeleteCriticalSection(&cs_find_end);
